...
var orderedBids = bids.sorted( isOrderedBefore: {(bid1: Double, bid2:Double) -> Bool in
    return  bid2 > bid1
})
...
var youngestToOldest = birthYears.sorted(isOrderedBefore: { (year1: Int, year2: Int) -> Bool in
    return year1 > year2
})
...
var alphabeticalSoup = soup.sorted(isOrderedBefore: { (soup1: String, soup2: String) -> Bool in
    return soup2 > soup1
})
...
var alphabeticalSoups = soups.sorted(isOrderedBefore: {(soup1: String, soup2: String) -> Bool in
    return soup2 > soup1
})
